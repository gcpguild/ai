...
JSON file input: Indias_Uttar-Pradesh_62d5881479627ca39f53c64a.json
...
1)	Indias_Uttar-Pradesh_Temples_Google_SerpApi.csv
2)	Indias_Uttar-Pradesh_Google_Paginations_Links_SerpApi.csv
...
https://github.com/gcpguild/search/blob/main/parseserpapijson.py
...
"""
#-----------------------------------------------------#-----------------------------------------------------
githubserpaijson = "https://github.com/gcpguild/search/blob/main/parseserpapijson.py"
...
def gematerials(data,di):
    ...
    if (di == "pagination"):
        ...
        serpapilinks.append(data[di]['other_pages']['20'])
    temdat =  []
...
for cc in range(0,csco):
    ...
    ImginlinepslistSerplinks = serapitemplesoutfile(st = statename,csvf ="Google_Image_Links_SerpApi.csv")
...
#-----------------------------------------------------
cleanimginlinepslistout = []

for m in range(0,len(imginlinepslist)):
    cleanimginlinepslistout.append(imginlinepslist[m])
...
#-----------------------------------------------------
tfm = pd.DataFrame(cleanimginlinepslistout)

mudim =  ['Images SerApi Links']
modem = 'w' if mudim else 'a'

tfm.to_csv(ImginlinepslistSerplinks, encoding='utf-8', mode=modem, header=mudim)

templescsvimagefile = Path(ImginlinepslistSerplinks)

if  templescsvimagefile.is_file():
    pi="\'SerpApi Image Links data : \' :"
    p = ("{} {}".format(pi,templescsvgooglefile))
    prt(p)
#-----------------------------------------------------